...
*   **`piMitters` (Raspberry Pi 5 Server):** Runs `nixos-25.11` (Stable) for hosting production bots (`geminiOS`, `milton`), Shiny Server, RevealJS static slides, and GitHub Actions self-hosted runners.
...
│   ├── piMitters/                # Headless Raspberry Pi 5 server configuration
│   │   ├── configuration.nix
│   │   ├── hardware-configuration.nix
...
└── users/
    └── kyle/
        ...
        ├── piMitters.nix         # piMitters-specific user packages & systemd timers/services
...
2.  **Server (`piMitters`):** Uses the stable `release-25.11` Home Manager branch following the `nixos-raspberrypi` hardware wrapper pin. This ensures server predictability, stable kernel module building, and dependency stability.
...
### Server (`piMitters`)
*   **Production Services:** The system configuration compiles a unified `rEnv` defined in [hosts/piMitters/r-env.nix](file:///home/kyle/NixOS/hosts/piMitters/r-env.nix).
...
*   [piMitters-bootstrap.md](file:///home/kyle/NixOS/piMitters-bootstrap.md) (This file is ignored by Git and not committed to the repository).
...
*   **System-wide Packages:** Edit `environment.systemPackages` in the host's `configuration.nix` (e.g., [hosts/nixMitters/configuration.nix](file:///home/kyle/NixOS/hosts/nixMitters/configuration.nix) or [hosts/piMitters/configuration.nix](file:///home/kyle/NixOS/hosts/piMitters/configuration.nix)).
*   **User/Home Manager Packages:** Edit `home.packages` in the base [users/kyle/home.nix](file:///home/kyle/NixOS/users/kyle/home.nix), or host-specific user config [users/kyle/nixMitters.nix](file:///home/kyle/NixOS/users/kyle/nixMitters.nix) / [users/kyle/piMitters.nix](file:///home/kyle/NixOS/users/kyle/piMitters.nix).
...
*   **Production Server (`piMitters`):**
    1. Open [hosts/piMitters/r-env.nix](file:///home/kyle/NixOS/hosts/piMitters/r-env.nix).
    2. Add or remove packages from the `packages` list (must be valid Nixpkgs R packages, e.g., `dplyr`, `ggplot2`).
    3. Rebuild the system: `sudo nixos-rebuild switch --flake ~/NixOS#piMitters`.
*   **Laptop Workstation (`nixMitters`):**
...
When a new NixOS stable branch is released (e.g., upgrading `release-25.11` to `release-26.05` on `piMitters`):
...
4. Rebuild the system: `sudo nixos-rebuild switch --flake ~/NixOS#piMitters`.
   > [!IMPORTANT]
