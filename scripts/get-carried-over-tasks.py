...
def get_tasks(simulated_date_str=None):
    ...
    today_filenames = {
        today_filename_part1.lower(),
        today_filename_part2.lower(),
    }
        ...
        for root, dirs, files in os.walk(DAILY_NOTES_DIR):
            for file in files:
                ...
                for line in lines:
                    ...
                    if is_todo and not is_indented:
                        ...
                        is_habit = (
                            current_heading
                            and "daily habit" in current_heading
                        )
                        if is_habit:
                            continue
                        if (
                            current_heading
                            and "personal todo" in current_heading
                        ):
                            personal_todos.append(task_text)
                        elif (
                            current_heading
                            and "work todo" in current_heading
                        ):
            ...
            for file in files:
                ...
                for line in lines:
                    ...
                    if is_todo and not is_indented:
                        ...
                        if task_text:
                            has_tag = any(
                                tag in task_text.lower() for tag in WORK_TAGS
                            )
                            if has_tag:
                                work_todos.append(task_text)
