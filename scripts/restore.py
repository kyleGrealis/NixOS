...
def restore_nixPi5(backup_dir: Path, dry_run: bool = False) -> None:
    ...
    # Skip Home Manager managed dotfiles (.bashrc, .bash_profile, .bash_aliases, .gitconfig) to prevent activation conflicts.
    # These are declaratively generated by Home Manager via home.nix.
    dotfiles = [".Rprofile", ".Renviron", ".env"]
    for dot in dotfiles:
    ...
    # Skip settings.json files managed via Home Manager out-of-store symlinks to avoid conflicts.
    # They point to ~/Documents/obsidian/dev/agent-guidelines/settings/ and are restored via the Obsidian vault.
...
def restore_nixMitters(backup_dir: Path, dry_run: bool = False) -> None:
    ...
    # Skip settings.json files managed via Home Manager out-of-store symlinks to avoid conflicts.
    # On nixMitters, these symlinks point to ~/Documents/obsidian/dev/agent-guidelines/settings/ which is synced via Syncthing.
